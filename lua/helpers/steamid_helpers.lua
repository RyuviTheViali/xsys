...
function AbitraryPrecision.__MakeTable(v)
	local v,t = tostring(v),{}
	for k=1,v:len() do t[k] = v:sub(k,k) end
	return t
end
...
function AbitraryPrecision.Add(a,b)
	...
	local pos1,pos2,pos3,C,overflow = #A,#B,1,{},0
...
local accountids={}
function util.AccountIDFromSteamID(steamid)
	local cached = accountids[ steamid]
	if cached==false then
	elseif cached then
		return cached
	end
	
	local X,Y,Z=steamid:match("^STEAM%_(%d):(%d):(%d+)$")
	
	if not Z then
		accountids[steamid] = false
		return
	end
	
	local account_id = Z*2 + Y
	accountids[steamid] = account_id
	return account_id
end

local Player=FindMetaTable"Player"

Player.AccountID=function(self)
